...
function WorkflowCanvasInner({ workspaceId, workflow }: WorkflowCanvasProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full min-h-0 w-full min-w-0 overflow-hidden bg-stone-950"
    ...
    >
      ...
      >
        <Background gap={24} size={1} color="currentColor" className="text-stone-800" />
        <MiniMap
          pannable
          zoomable
          className="!bottom-4 !left-4 !border !border-stone-700 !bg-stone-900"
          nodeColor="#0f172a"
          maskColor="rgba(148, 163, 184, 0.08)"
        />
        <Controls position="top-right" showInteractive={false} className="!rounded-md !border !border-stone-700 !bg-stone-900" />
        <Panel position="top-left">
          <div className="rounded-md border border-stone-800 bg-stone-900 px-3 py-2 text-xs text-stone-300 shadow-lg shadow-black/20">
            {selectedNodeCount > 0
        ...
        {connectionFeedback ? (
          <Panel position="bottom-center">
            <div className="rounded-md border border-red-900 bg-red-950/50 px-3 py-2 text-xs font-medium text-red-200 shadow-lg shadow-black/20">
              {connectionFeedback}
            </div>
          </Panel>
        ) : null}
      ...
      {nodes.length === 0 ? (
        <div className="pointer-events-none absolute inset-0 grid place-items-center">
          <div className="rounded-lg border border-dashed border-stone-700 bg-stone-900/90 px-6 py-5 text-center shadow-lg shadow-black/20">
            <p className="text-sm font-medium text-stone-50">Blank canvas</p>
